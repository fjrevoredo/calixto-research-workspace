...
pytestmark = [pytest.mark.installer_unix]


def require_unix_prereqs() -> None:
    if sys.platform == "win32":
        pytest.skip("install.sh is required in Linux CI, not on Windows hosts")
    if not have_git_bash() or shutil.which("git") is None:
        if os.environ.get("CALIXTO_ENFORCE_INSTALLER_PREREQS") == "1":
            pytest.fail("bash and git must be available for installer_unix tests")
        pytest.skip("bash and git are required for installer_unix tests")
...
def test_fresh_install_copies_root_config_and_writes_managed_entries(
    ...
    result = invoke_unix_installer(
        ...
        target,
        env,
        args=["--non-interactive", "--skip-deps"],
    )
...
def test_dry_run_creates_no_installer_artifacts(tmp_path: Path) -> None:
    ...
    fresh = invoke_unix_installer(
        checkout,
        fresh_target,
        fresh_env,
        args=["--dry-run", "--non-interactive", "--skip-deps"],
    )
