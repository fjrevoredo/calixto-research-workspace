...
def windows_cmd_contents() -> str:
    return (
        ...
        "if errorlevel 1 (\n"
        "  echo calixto: neither pwsh nor powershell is available to run \"%CALIXTO_LAUNCHER_PS1%\" 1>&2\n"
        "  exit /b 1\n"
        ")\n"
        "powershell -NoProfile -ExecutionPolicy Bypass -File \"%CALIXTO_LAUNCHER_PS1%\" %*\n"
        "exit /b %ERRORLEVEL%\n"
    )
...
def windows_ps1_contents() -> str:
    return (
        ...
        "        $parent = if ($parent) { $parent.FullName } else { $null }\n"
